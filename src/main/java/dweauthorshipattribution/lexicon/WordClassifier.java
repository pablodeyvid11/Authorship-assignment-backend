...
//			Word wrd = new Word(word, WordIF.WordClassification.FINAL_PONCTUATION);
...
//
//			Word wrd = new Word(word, WordIF.WordClassification.INTERMEDIATE_PONTUATION);
...
//            wrd = new Word(palavra, WordIF.WordClassification.NOUN);
			...
			// System.out.println("PALAVRA : "+wrd.getWord()+" classificada como
			// :"+wrd.getWordClassification().toString());
			// buscaNaNet
...
//           System.out.println("Palavra : "+wrd.getWord()+ " já consta no banco de dados como :"+wrd.getWordClassification().toString());
		...
		return wrd;
		/***
		 * 1. Vai no banco e verifica se a palavra ja se encontra (metodo getWord da
		 * classe WordDAO) 2. Se contiver, ja retorna-a 3. Se nao contiver a palavra,
		 * vai no dicionario online e busca 4. Classifica a palavra e de acordo com o
		 * enum WordIF.WordClassification 5. Insere a palavra no banco (metodo save do
		 * WordDAO) 6. Retorna a palavra
		 ***/
