 ...
 * compression models from {@link #main}. Behavior is specified with the
 * following arguments.
 * <UL>
 * <LI><code>-f   <i>FileName</i>: </code> Test specified file.</LI>
 * <LI><code>-s   <i>Integer</i>: </code> Sized tests up to specified number of
 * bytes.</LI>
 * <LI><code>-g: </code> Run small tests.</LI>
 * <LI><code><i>String</i>: </code> Test specified string.</LI>
 * <LI><code>-c   <i>Directory</i>: </code> Test calgary corpus found in
 * specified directory.</LI>
 * <LI><code>-x   <i>Directory</i>: </code> Test xml corpus found in specified
 * directory.</LI>
 * </UL>
 * <P>
 * The Calgary corpus can be downloaded from: <blockquote>
 * <a href="ftp://ftp.cpsc.ucalgary.ca/pub/projects/text.compression.corpus">
 * ftp://ftp.cpsc.ucalgary.ca/pub/projects/text.compression.corpus </a>.
 * </blockquote>
 * </P>
 * <P>
 * Because of the use of statics, only a single test should be run per virtual
 * machine.
 *
...
public final class Test {
	 ...
	 * <LI><code>-c   <i>Directory</i>: </code> Test calgary corpus found in
	 * specified directory.</LI>
	 * <LI><code>-x   <i>Directory</i>: </code> Test James Cheney's XML corpus found
	 * in specified directory.</LI>
	 * </UL>
	 * 
	 * @param args Parameters in fixed order.
	 * @throws IOException If there is an underlying I/O exception during
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < args.length; ++i) {
			if (args[i].equals("-f"))
				...
				testSize(Integer.parseInt(args[++i]));
	...
	/**
	 * Read all of the input from the given input stream and write it to the given
	 * output stream.
	 * 
	 * @param in  Input stream from which to read.
	 * @param out Output stream to which to write.
	 * @throws IOException If there is an exception reading or writing on the given
	 *                     streams.
	 */
	...
	/**
	 * Runs tests from 1 to give size, increasing size by a factor of two at each
	 * step. For each size, a test is made of a constant string consisting of
	 * repetitions of a single character, and a test of a random sequence of letters
	 * and then a random sequence of bytes.
	 * 
	...
	/**
	 * Tests compression/decompression of a given string. String is first rendered
	 * as bytes, given current localized default; see
	...
	/**
	 * Run a test of PPM on the specified bytes using a model of the specified
	 * order.
	 * 
	...
	private static boolean testPPMBytes(byte[] bytes, int order) throws IOException {
		return testBytes(bytes, new PPMModel(order), new PPMModel(order),
				"PPM(" + order + ")" + (order < 10 ? "  " : " "));
	}
	...
	/**
	 * Tests specified sequence of bytes with specified models for input and output,
	 * and specified name.
	 * 
	 ...
	 * @throws IOException If there is an underlying I/O exception.
	 */
	private static boolean testBytes(byte[] bytes, ArithCodeModel modelIn, ArithCodeModel modelOut, String name)
			throws IOException {
	...
	/**
	 * Returns a string representation of the compression rate indicated by the
	 * specified number of original bytes and compressed bytes. Expressed in bits
	 * per byte.
	 * 
	...
	/**
	 * Convert an integer to a string, padding with spaces in the front to provide a
	 * result of the specified minimum length.
	 * 
	 * @param n         Integer to convert to string.
	 * @param minLength Minimum length of result.
	 * @return String representation of integer, padded to at least specified
	 *         length.
	 */
	...
	/**
	 * Returns a string representation of the speed of compression indicated by the
	 * specified number of original bytes and time in milliseconds.
	 * 
	...
	/**
	 * Truncates string to printable length, appending epenthetic dots if it is
	 * truncated.
	 * 
	...
	/**
	 * Fills the specified byte array with random alphanumeric characters generated
	 * by the specified randomizer.
	 * 
	...
	/**
	 * Generates the next random byte between the specified low and high bytes
	 * inclusive, using the specified randomizer.
	 * 
	 * @param r    Randomizer.
	...
	/**
	 * Returns next random alphabetic or numeric byte as determined by the specified
	 * randomizer.
	 * 
	 * @param r Randomizer.
	...
	private static byte nextRandomAlphaNum(Random r) {
		if (r.nextBoolean())
			return nextByteRange(r, (byte) 'a', (byte) 'z');
		if (r.nextBoolean())
			return nextByteRange(r, (byte) 'A', (byte) 'Z');
